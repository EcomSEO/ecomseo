...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata(locale as Locale, "/bigcommerce-seo");
}

export default async function BigCommerceSeoPage({
  params,
}: {
    ...
    <>
      <JsonLd
        data={serviceJsonLd({
          ...
          name: "BigCommerce SEO for High-SKU Catalogs",
          description: "Many SKUs? No worries, we got it covered. We optimize BigCommerce stores for crawlability, speed, and organic revenue growth.",
          path: "/bigcommerce-seo",
        })}
      />
      <JsonLd
        data={breadcrumbJsonLd(locale as Locale, [
          { name: "Home", path: "/" },
          { name: "BigCommerce SEO for High-SKU Catalogs", path: "/bigcommerce-seo" },
        ])}
      />
      <ServicePageTemplate
      hero={{
