...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata(locale as Locale, "/resources/library/on-page-checklist");
}


export default async function OnPageChecklistPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  ...
  return (
    <>
      <JsonLd data={breadcrumbJsonLd(locale as Locale, [
        ...
        { name: "On-Page Checklist", path: "/resources/library/on-page-checklist" },
      ])} />
      <Navigation />
      <main className="min-h-screen bg-white text-gray-900">
        <div className="mx-auto max-w-[1000px] px-5 md:px-10 pt-[120px] md:pt-[140px] pb-20">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-400 mb-6">
            ...
            <span>/</span>
            <span className="text-gray-600">{t.onPageChecklist.title}</span>
          </div>
