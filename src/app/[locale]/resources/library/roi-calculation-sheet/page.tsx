...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata(locale as Locale, "/resources/library/roi-calculation-sheet");
}


export default async function RoiCalculationPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = libraryDetailT[locale as Locale];

  const relatedResources = [
    {
      title: t.relatedResources.abcTemplate.title,
      description: t.relatedResources.abcTemplate.description,
  ...
  return (
    <>
      <JsonLd data={breadcrumbJsonLd(locale as Locale, [
        ...
        { name: "ROI Calculation Sheet", path: "/resources/library/roi-calculation-sheet" },
      ])} />
      <Navigation />
      <main className="min-h-screen bg-white text-gray-900">
        <div className="mx-auto max-w-[1000px] px-5 md:px-10 pt-[120px] md:pt-[140px] pb-20">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-400 mb-6">
            ...
            <span>/</span>
            <span>{t.breadcrumbs.templates}</span>
            <span>/</span>
            <span className="text-gray-600">{t.roiSheet.title}</span>
          </div>
