...
export default async function AcademyPage({
  ...
  // Count total topics
  const totalTopics = t.clusters.reduce((sum, c) => sum + c.topics.length, 0);
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-bg">
          ...
          <div className="mx-auto max-w-[1200px] flex flex-col items-center text-center gap-6">
            ...
            <div className="flex items-center gap-6 mt-2">
              <div className="flex items-center gap-2 text-sm">
                <span className="text-accent font-semibold">{t.clusters.length}</span>
                <span className="text-body">phases</span>
              </div>
              <div className="w-px h-4 bg-border" />
              <div className="flex items-center gap-2 text-sm">
                <span className="text-accent font-semibold">{totalTopics}</span>
                <span className="text-body">{t.topicsCount}</span>
              </div>
              <div className="w-px h-4 bg-border" />
              <div className="flex items-center gap-2 text-sm">
        ...
        {/* Intro */}
        <section className="w-full px-5 md:px-16 pb-12">
        ...
        <section className="w-full px-5 md:px-16 pb-20">
          <div className="mx-auto max-w-[900px]">
            ...
            <div className="flex items-center gap-3 mb-10">
              ...
              <h2 className="text-heading text-xl font-medium tracking-[-0.02em]">
                The Ecommerce SEO Learning Roadmap
              </h2>
            </div>
            ...
            <div className="relative">
              {t.clusters.map((cluster, i) => {
                const isLast = i === t.clusters.length - 1;
                return (
                  <div key={cluster.title} className="relative pb-10 last:pb-0">
                    {/* Vertical connector line */}
                    {!isLast && (
                      <div className="absolute left-[23px] top-[56px] bottom-0 w-px bg-gradient-to-b from-accent/30 to-border hidden md:block" />
                    )}

                    {/* Phase header row */}
                    <div className="flex items-start gap-4 mb-4">
                      {/* Step number circle */}
                      <div className="flex items-center justify-center w-[48px] h-[48px] rounded-full bg-accent/15 border-2 border-accent/30 shrink-0">
                        <span className="text-accent font-bold text-sm">{i + 1}</span>
                      </div>

                      <div className="flex-1 pt-1">
                        <div className="flex flex-wrap items-center gap-3 mb-1">
                          <h3 className="text-heading text-lg font-medium tracking-[-0.02em]">
                            {cluster.title}
                          </h3>
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium border ${difficultyColors[cluster.difficulty]}`}>
                        ...
                        <p className="text-body text-sm leading-relaxed mb-4 max-w-[600px]">
                          {cluster.description}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="md:ml-[64px] rounded-xl border border-border overflow-hidden bg-bg-card">
                      {cluster.topics.map((topic, j) => {
                        ...
                        const isLastTopic = j === cluster.topics.length - 1;
                        return (
                          <LocaleLink
                            key={href}
                            href={href}
                            className={`flex items-center gap-3 px-4 py-3 hover:bg-white/[0.04] transition-colors group ${!isLastTopic ? "border-b border-border" : ""}`}
                          >
                            <span className="flex items-center justify-center w-6 h-6 rounded bg-accent/10 text-accent text-xs font-semibold shrink-0">
                              {j + 1}
                            </span>
                    ...
                    {!isLast && (
                      <div className="flex justify-center md:justify-start md:ml-[23px] py-2">
