...
export default async function EcommerceSeoServicesPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  const t = servicePageData["ecommerce-seo-services"][locale as Locale];
  return (
    <>
      <JsonLd
        data={serviceJsonLd({
      ...
      <JsonLd
        data={breadcrumbJsonLd(locale as Locale, [
          ...
          { name: t.hero.badge, path: "/ecommerce-seo-services" },
        ])}
      />
