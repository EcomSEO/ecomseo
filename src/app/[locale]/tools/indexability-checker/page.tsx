...
import IndexabilityCheckerClient from "@/components/tools/IndexabilityCheckerClient";
import { buildPageMetadata } from "@/lib/i18n/metadata";
import type { Locale } from "@/lib/i18n/config";
import { indexabilityCheckerT } from "@/lib/i18n/translations/seoTools";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata(locale as Locale, "/tools/indexability-checker");
}

export default async function IndexabilityCheckerPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  const t = indexabilityCheckerT[locale as Locale];
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-bg">
        ...
        <ToolPreviewSection
          src="/images/tools/indexability-checker-preview.png"
          imgWidth={1800}
          imgHeight={934}
          url="ecomseo.co/tools/indexability-checker"
