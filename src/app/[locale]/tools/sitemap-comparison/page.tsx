...
import SitemapComparisonClient from "@/components/tools/SitemapComparisonClient";
import { buildPageMetadata } from "@/lib/i18n/metadata";
import type { Locale } from "@/lib/i18n/config";
import { sitemapComparisonT } from "@/lib/i18n/translations/seoTools";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata(locale as Locale, "/tools/sitemap-comparison");
}

export default async function SitemapComparisonPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  const t = sitemapComparisonT[locale as Locale];
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-bg">
        ...
        <ToolPreviewSection
          src="/images/tools/sitemap-comparison-preview.png"
          imgWidth={1800}
          imgHeight={2098}
          url="ecomseo.co/tools/sitemap-comparison"
