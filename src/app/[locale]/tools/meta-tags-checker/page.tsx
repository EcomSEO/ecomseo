...
import MetaTagsCheckerClient from "@/components/tools/MetaTagsCheckerClient";
import { buildPageMetadata } from "@/lib/i18n/metadata";
import type { Locale } from "@/lib/i18n/config";
import { metaTagsCheckerT } from "@/lib/i18n/translations/seoTools";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata(locale as Locale, "/tools/meta-tags-checker");
}

export default async function MetaTagsCheckerPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  const t = metaTagsCheckerT[locale as Locale];
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-bg">
        ...
        <ToolPreviewSection
          src="/images/tools/meta-tags-checker-preview.png"
          imgWidth={1800}
          imgHeight={1010}
          url="ecomseo.co/tools/meta-tags-checker"
