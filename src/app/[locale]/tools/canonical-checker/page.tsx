...
import CanonicalCheckerClient from "@/components/tools/CanonicalCheckerClient";
import { buildPageMetadata } from "@/lib/i18n/metadata";
import type { Locale } from "@/lib/i18n/config";
import { canonicalCheckerT } from "@/lib/i18n/translations/seoTools";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata(locale as Locale, "/tools/canonical-checker");
}

export default async function CanonicalCheckerPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  const t = canonicalCheckerT[locale as Locale];
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-bg">
        ...
        <ToolPreviewSection
          src="/images/tools/canonical-checker-preview.png"
          imgWidth={1280}
          imgHeight={3361}
          url="ecomseo.co/tools/canonical-checker"
