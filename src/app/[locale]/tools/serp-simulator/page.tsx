...
import SerpSimulatorClient from "@/components/tools/SerpSimulatorClient";
import { buildPageMetadata } from "@/lib/i18n/metadata";
import type { Locale } from "@/lib/i18n/config";
import { serpSimulatorT } from "@/lib/i18n/translations/seoTools";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata(locale as Locale, "/tools/serp-simulator");
}

export default async function SerpSimulatorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  const t = serpSimulatorT[locale as Locale];
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-bg">
        ...
        <ToolPreviewSection
          src="/images/tools/serp-simulator-preview.png"
          imgWidth={1280}
          imgHeight={2837}
          url="ecomseo.co/tools/serp-simulator"
